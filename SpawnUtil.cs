...
namespace FunCommand
{
    public class SpawnUtil
    {
        ...
        /// <para>ignoreTile: 该type的NPC在生成时不会检测空间是否足够</para>
        ...
        public struct SpawnBatchPool
        {
            ...
            public void Initialize(int poolLength)
            {
                ...
                ignoreTile = new bool[poolLength];
            ...
            public void Set(bool setRandomType = false, int setTotalAmount = 0, int[] setType = default, int[] setAmount = default, int[] setWeight = default, bool[] setGreedy = default, bool[] setOverlap = default, bool[] setIgnoreTile = default, float[] setAI0 = default, float[] setAI1 = default, float[] setAI2 = default, float[] setAI3 = default)
            {
                ...
                for (int i = 0; i < length; i++)
                {
                    ...
                    if (setIgnoreTile != default)
                        ignoreTile[i] = setIgnoreTile[i];
                    if (setAI0 != default)
                        ai0[i] = setAI0[i];
                    if (setAI1 != default)
                        ai1[i] = setAI1[i];
        ...
        public static NPC[] SpawnNPCBatch(IEntitySource source, Rectangle whiteArea, Rectangle blackArea, SpawnBatchPool pool)
        {
            ...
            List<Point> ignoringTile = new List<Point>();
            ...
            for (int n = 0; n <= (pool.randomType ? pool.totalType - 1 : pool.length); n++) // 尝试 length 或 totalAmount 次
            {
                Task taskPerType = new Task(() =>
                {
                    ...
                    bool canSpawn;
                    if (pool.ignoreTile[index])
                    {
                        spawnPoint = ignoringTile;
                    }
                    else lock (spaceToPoint)
                    {
